...
def get_model(config):
    ...
    model = LSTM(config["enc_in"], config["enc_in"], config["hs"], config["hl"])
...
def run_validation(model, val_dataloader, device):
    ...
    for batch_idx, batch in enumerate(val_dataloader):
        ...
        with torch.no_grad():
            output, outputs_debug = model.test_data(data, config["pred_len"], device)
            loss += loss_fn(output, label)
            loss_debug += loss_fn_debug(label[:, 0, :], output[:, 0, :])
    plt.figure()
    ...
    i = 0
    j = 0
    plt.plot(x, 1 + label[0, :, (i + j * 4) * 2].cpu().numpy(), label="label")
    plt.plot(x, 1 + output[0, :, (i + j * 4) * 2].cpu().numpy(), label="output")
    plt.plot(outputs_debug[0, :, (i + j * 4) * 2].cpu().numpy(), label="outputs_debug")
    # plt.plot(
    #     0.1 + output_debug_2[0, :, (i + j * 4) * 2].cpu().numpy(),
    #     label="output_debug_2",
    # )
    plt.plot(0.2 + H_seq[0, :, j, i].cpu().numpy(), label="H_seq")
    plt.plot(0.3 + H[0, :, j, i].cpu().numpy(), label="H")
    plt.legend()
    plt.xlim([75, 96])
    plt.savefig("output_eval.png")
    plt.close()
...
if __name__ == "__main__":
    ...
    config["model_folder"] = config["model_folder"] + "/LSTM"
    config["experiment_name"] = (
        "/".join(config["experiment_name"].split("/", 1)) + "/LSTM" + model_name
    )
    ...
    with open("loss_LSTM.pkl", "wb") as f:
