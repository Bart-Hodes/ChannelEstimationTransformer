...
from TransformerModel.encoder import Encoder, EncoderBlock
from TransformerModel.decoder import Decoder, DecoderBlock, ProjectionLayer
from TransformerModel.embed import InputEmbeddings, PositionalEncoding
...
class Transformer(nn.Module):
    ...
    def encode(self, src, src_mask=None, first_inference=True):
        # (batch, seq_len, d_model)
        src = self.src_embed(src)
        src = self.src_pos(src, self.position_counter)
        # self.position_counter += 1
        return self.encoder(src, src_mask, first_inference=first_inference)
    ...
    def forward(self, encoder_input, decoder_input, first_inference=True):

        # Run the tensors through the encoder, decoder and the projection layer
        encoder_output = self.encode(
            encoder_input, first_inference=first_inference
        )  # (B, seq_len, d_model)
...
def build_transformer(
    ...
    for _ in range(N):
        encoder_self_attention_block = MultiHeadAttentionBlock(
            d_model, h, dropout, _, shift_size
        )
    ...
    decoder_blocks = []
    for _ in range(N):
        decoder_self_attention_block = MultiHeadAttentionBlock(d_model, h, dropout, 10)
        decoder_cross_attention_block = MultiHeadAttentionBlock(d_model, h, dropout, 10)
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
