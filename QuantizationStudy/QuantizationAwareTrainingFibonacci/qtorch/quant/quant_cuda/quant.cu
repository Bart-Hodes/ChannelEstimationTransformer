...
float findNearestNumberHost(float n, const float *fibonacci, int size)
{
...
Tensor fibonacci_quantize_partial_proximal_cuda(Tensor a, int wl, int fl, bool use_clamp, bool symmetric, float percentage)
{
    // use external random number right now
    ...
    std::vector<float> fibbinary = generatefibbinary(wl);

    for (auto &num : fibbinary)
    {
        num /= (1 << fl);
    }
    ...
    {
        quantized_values[i] = findNearestNumberHost(float_list[i], fibbinary.data(), fibbinary.size());
    }
    ...
    // //   count number of weights that are quantizec
    // int count = 0;
    // for (int64_t i = 0; i < size; i++)
    // {
    ...
    //     }
    // }
    // std::cout << "Number of weights quantized: " << count << std::endl;
    // std::cout << "Total number of weights: " << size << std::endl;
    // std::cout << "proximal" << std::endl;

    // Convert codewords to array
    float *d_fibbinary;
    cudaMalloc(&d_fibbinary, fibbinary.size() * sizeof(float));
    cudaMemcpy(d_fibbinary, fibbinary.data(), fibbinary.size() * sizeof(float), cudaMemcpyHostToDevice);

    fibonacci_quantize_partial_proximal_kernel<<<blockNums, blockSize>>>(a.data_ptr<float>(),
                                                                         o.data_ptr<float>(),
                                                                         size,
                                                                         d_fibbinary,
                                                                         fibbinary.size(),
...
Tensor fibonacci_quantize_partial_distant_cuda(Tensor a, int wl, int fl, bool use_clamp, bool symmetric, float percentage)
{
    // use external random number right now
    Tensor o = at::zeros_like(a);
    int64_t size = a.numel();
    ...
    fibonacci_quantize_partial_distant_kernel<<<blockNums, blockSize>>>(a.data_ptr<float>(),
                                                                        o.data_ptr<float>(),
                                                                        size,
                                                                        d_fibbinary,
                                                                        fibbinary.size(),
...
Tensor fibonacci_quantize_partial_stochastic_cuda(Tensor a, int wl, int fl, bool use_clamp, bool symmetric, float percentage)
{
    // use external random number right now
    Tensor o = at::zeros_like(a);
    int64_t size = a.numel();
    ...
    fibonacci_quantize_partial_stochastic_kernel<<<blockNums, blockSize>>>(a.data_ptr<float>(),
                                                                           o.data_ptr<float>(),
                                                                           size,
                                                                           d_fibbinary,
                                                                           fibbinary.size(),
