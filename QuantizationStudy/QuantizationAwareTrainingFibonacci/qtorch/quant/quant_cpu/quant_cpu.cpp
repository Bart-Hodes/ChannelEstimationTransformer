...
std::vector<float> generateFibbinary(int length)
{
...
int findNearestNumber(int n, const std::vector<float> &fibbinary)
{
    float nearest = fibbinary[0];
    for (int i = 1; i < fibbinary.size(); ++i)
    {
        if (std::abs(n - fibbinary[i]) < std::abs(n - nearest))
        {
            nearest = fibbinary[i];
        }
    }
...
Tensor fibonacci_quantize_partial_nearest(Tensor a, int wl, int fl, bool clamp, bool symmetric)
{
    CHECK_INPUT(a);
    auto a_array = a.data_ptr<float>();
    Tensor o = zeros_like(a);
    ...
    std::vector<float> fibbinary = generateFibbinary(wl); // Pass the correct number of arguments to generateFibonacci

    // // Convert from unsigned to signed representation
    // for (int i = 0; i < fibbinary.size(); ++i) {
    //     fibbinary[i] -= pow(2, wl-1);
    // }

    // Shift the fixed point
    for (int i = 0; i < fibbinary.size(); ++i)
    {
        fibbinary[i] /= pow(2, fl);
    }

    std::cout << "Fibonacci sequence: ";
    for (int i = 0; i < fibbinary.size(); i++)
    {
        std::cout << fibbinary[i] << " ";
    }

    t_min = *std::min_element(fibbinary.begin(), fibbinary.end());
    t_max = *std::max_element(fibbinary.begin(), fibbinary.end());

    for (int64_t i = 0; i < size; i++)
    {
        int nearest_fib = findNearestNumber((float)a_array[i], fibbinary);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def("fibonacci_quantize_partial_nearest", &fibonacci_quantize_partial_nearest, "Fixed Point Number Nearest Neighbor Quantization (CPU)");
