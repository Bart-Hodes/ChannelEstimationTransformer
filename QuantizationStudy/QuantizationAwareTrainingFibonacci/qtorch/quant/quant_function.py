...
def fibonacci_quantize_partial(
    x, wl, fl, clamp=True, symmetric=False, percentage=1, rounding="distant"
):
    """
    Quantize a single precision Floating Point into low-precision Fixed Point
    ...
    if rounding == "proximal":
        out = quant_module.fibonacci_quantize_partial_proximal(
            x.contiguous(), wl, fl, clamp, symmetric, percentage
        )
    elif rounding == "distant":
        out = quant_module.fibonacci_quantize_partial_distant(
            x.contiguous(), wl, fl, clamp, symmetric, percentage
        )
    elif rounding == "stochastic":
        out = quant_module.fibonacci_quantize_partial_stochastic(
            x.contiguous(), wl, fl, clamp, symmetric, percentage
        )
    return out
