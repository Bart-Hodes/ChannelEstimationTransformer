...
class OptimLP(Optimizer):
    ...
    def __init__(
        ...
        missing_keys = [
            key
            for key in self.quantization_settings.keys()
            if key not in dict(self.named_parameters)
        ]
        if missing_keys:
            print(
                "The following parameters specified in quantizationSettings are not present in the model:"
            )
            for key in missing_keys:
                print(key)
            raise ValueError(
                "Not all parameters specified in quantizationSettings are present in the model."
            )
    ...
    def step(self, percentage=None, closure=None):
        ...
        for name, param in self.named_parameters:
            if (
                name in self.quantization_settings
                and "weight_quant" in self.quantization_settings[name]
            ):
