...
def fixed_point_quantize(x, wl, fl, clamp=True, symmetric=False, rounding="stochastic"):
    ...
    assert rounding in ["stochastic", "proximal"]
    assert_wl_fl(wl, fl)
    quant_module = get_module(x)
...
def fixed_point_quantize_partial(
    x, percentage, wl, fl, clamp=True, symmetric=False, rounding="stochastic"
):
    """
    Quantize a single precision Floating Point into low-precision Fixed Point
    ...
    """
    assert isinstance(x, torch.Tensor)
    assert rounding in ["distant", "proximal", "stochastic"]
    assert_wl_fl(wl, fl)
    quant_module = get_module(x)
    if rounding == "proximal":
        out = quant_module.fixed_point_quantize_partial_proximal(
            x.contiguous(), wl, fl, clamp, symmetric, percentage
        )
    elif rounding == "distant":
        out = quant_module.fixed_point_quantize_partial_distant(
            x.contiguous(), wl, fl, clamp, symmetric, percentage
        )
    elif rounding == "stochastic":
        out = quant_module.fixed_point_quantize_partial_stochastic(
            x.contiguous(), wl, fl, clamp, symmetric, percentage
        )
    return out
