...
Tensor fixed_point_quantize_partial_cuda(Tensor a, int wl, int fl, bool use_clamp, bool symmetric, float percentage) {
  // use external random number right now
  ...
  // // Step 3: Evaluate the difference against the threshold and count the number of values being quantized
  // int num_quantized_values = 0;
  // for (int64_t i = 0; i < size; i++)
  // {
  //   if (fabs(float_list[i] - quantized_values[i]) <= threshold)
  //   {
  //     num_quantized_values++;
  //   }
  // }
  // std::cout << "Number of values being quantized: " << num_quantized_values << std::endl;
  // std::cout << "Threshold: " << threshold << std::endl;

  fixed_point_quantize_partial_kernel<<<blockNums, blockSize>>>(a.data_ptr<float>(),
                                                                o.data_ptr<float>(),
                                                                size,
                                                                sigma,
                                                                use_clamp,
