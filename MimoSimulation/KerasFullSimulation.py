...
class Model(tf.keras.Model):
    ...
    def __init__(
        ...
        self._num_ut_ant = (
            4  # Must be a multiple of two as dual-polarized antennas are used
        )
        self._num_bs_ant = (
            8  # Must be a multiple of two as dual-polarized antennas are used
        )
        ...
        if self._domain == "freq":
            ...
            self._l_min, self._l_max = time_lag_discrete_time_channel(
                self._rg.bandwidth
            )
    ...
    @tf.function(
        jit_compile=True
    )  # See the following guide: https://www.tensorflow.org/guide/function
    def call(self, batch_size, ebno_db):
        ...
        if self._domain == "time":
            ...
            cir = self._cdl(
                batch_size, self._rg.num_ofdm_symbols, 1 / self._rg.ofdm_symbol_duration
            )
