...
class OptimLP(Optimizer):
    ...
    def __init__(
        ...
        self.named_parameters = [x for x in named_parameters]
        # check if all parameters in quantization settings are present in the model
        missing_keys = [key for key in self.quantization_settings.keys() if key not in dict(self.named_parameters)]
        if missing_keys:
            print("The following parameters specified in quantizationSettings are not present in the model:")
            for key in missing_keys:
                print(key)
            raise ValueError("Not all parameters specified in quantizationSettings are present in the model.")
    ...
    def step(self, percentage = None, closure=None):
        """
        ...
        for name, param in self.named_parameters:
            if name in self.quantization_settings and "weight_quant" in self.quantization_settings[name]:
                quant_func = self.quantization_settings[name]["weight_quant"]
                if percentage is None:
                    param.data = quant_func(param.data)
                else:
                    param.data = quant_func(param.data, percentage)
    ...
    def setQuantizationSettings(self, quantization_settings):
        self.quantization_settings = quantization_settings
        # check if all parameters in quantization settings are present in the model
        missing_keys = [key for key in self.quantization_settings.keys() if key not in dict(self.named_parameters)]
        if missing_keys:
