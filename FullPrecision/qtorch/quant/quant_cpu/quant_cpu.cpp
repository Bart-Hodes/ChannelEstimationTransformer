...
Tensor fixed_point_quantize_partial_nearest(Tensor a, int wl, int fl, float percentage, bool clamp, bool symmetric)
{
  CHECK_INPUT(a);
  auto a_array = a.data_ptr<float>();
  Tensor o = zeros_like(a);
  ...
  for (int64_t i = 0; i < size; i++)
  {
    quantized_values[i] = round(a_array[i], 0.5, sigma);
  }

  // Step 2: Determine the threshold for the n% closest values
  int num_values_to_round = size * percentage;
  std::vector<float> differences(size);
  for (int64_t i = 0; i < size; i++)
  {
    differences[i] = fabs(a_array[i] - quantized_values[i]);
  }
  std::sort(differences.begin(), differences.end());
  float threshold = differences[num_values_to_round];

  // Step 3: Round only those values that fall within the threshold
  for (int64_t i = 0; i < size; i++)
  {
    if (fabs(a_array[i] - quantized_values[i]) <= threshold)
    {
      o_array[i] = round(a_array[i], 0.5, sigma);
    }
    else (o_array[i] = a_array[i]);
...
std::vector<double> generateFibonacci(int wl, int fl) {
  // Generate the first two numbers of the Fibonacci sequence
    std::vector<double> fibonacci;
    if (wl + fl >= 1)
        fibonacci.push_back(pow(2, -fl));
    if (wl + fl >= 2)
        fibonacci.push_back(pow(2, -fl));

  // Calculate the largest value in wl bits
    float max = pow(2, wl - fl - 1);
    std::cout << "Max: " << max << std::endl;
      
    while (fibonacci[fibonacci.size() - 1] + fibonacci[fibonacci.size() - 2] <= max) {
        double next = fibonacci[fibonacci.size() - 1] + fibonacci[fibonacci.size() - 2];
        fibonacci.push_back(next);
    }
    return fibonacci;
}

int findNearestNumber(int n, const std::vector<double>& fibonacci) {
    double nearest = fibonacci[0];
    for (int i = 1; i < fibonacci.size(); ++i) {
        if (std::abs(n - fibonacci[i]) < std::abs(n - nearest)) {
            nearest = fibonacci[i];
        }
    }
    return nearest;
...
Tensor fibonacci_quantize_nearest(Tensor a, int wl, int fl, bool clamp, bool symmetric)
{
CHECK_INPUT(a);
auto a_array = a.data_ptr<float>();
Tensor o = zeros_like(a);
...
std::cout << "wl: " << wl << " fl: " << fl << std::endl;

std::vector<double> fibonacci = generateFibonacci(wl, fl); // Pass the correct number of arguments to generateFibonacci
...
t_min = *std::min_element(fibonacci.begin(), fibonacci.end());
t_max = *std::max_element(fibonacci.begin(), fibonacci.end());


  for (int64_t i = 0; i < size; i++) {
      int nearest_fib = findNearestNumber((float)a_array[i], fibonacci);
      o_array[i] = nearest_fib;
      if (clamp) {
          o_array[i] = clamp_helper(o_array[i], t_min, t_max);
      }
  }
...
float findNearestNumber(float n, const std::vector<float>& fibonacci) {
    double nearest = fibonacci[0];
    for (int i = 1; i < fibonacci.size(); ++i) {
        if (std::abs(n - fibonacci[i]) < std::abs(n - nearest)) {
            nearest = fibonacci[i];
...
Tensor noAdjecentOnes_quantize_nearest(Tensor a, int wl, int fl, bool clamp, bool symmetric)
{
  CHECK_INPUT(a);
  auto a_array = a.data_ptr<float>();
  Tensor o = zeros_like(a);
...
//   }
  ...
  t_min = *std::min_element(codewords.begin(), codewords.end());
  t_max = *std::max_element(codewords.begin(), codewords.end());


  for (int64_t i = 0; i < size; i++) {
      float nearest_num = findNearestNumber((float)a_array[i], codewords);
      std::cout << "Nearest number: " << nearest_num << std::endl;
      std::cout << "Original number: " << a_array[i] << std::endl;
      o_array[i] = nearest_num;
      if (clamp) {
          o_array[i] = clamp_helper(o_array[i], t_min, t_max);
      }
  }
...
// Tensor grad_scale(Tensor x, float scale) {
//     auto y = x.clone();
//     auto y_grad = x * scale;
//     return y.detach() - y_grad.detach() + y_grad;
// }

// Tensor round_pass(Tensor x) {
//     auto y = x.round();
//     auto y_grad = x.clone();
//     return y.detach() - y_grad.detach() + y_grad;
// }
...
// Tensor LS_quantize_nearest(Tensor a, int wl, tensor s, bool clamp, bool symmetric)
// {
//   CHECK_INPUT(a);
//   auto a_array = a.data_ptr<float>();
//   Tensor o = zeros_like(a);
...
//   Qn = -pow(2, (wl - 1));
//   Qp = pow(2, (wl - 1)) - 1;
//   gradScaleFactor = 1 / sqrt(a.numel() * Qp);

//   s = grad_scale(s, gradScaleFactor);
//   a = a / s;
//   a = clamp(a, Qn, Qp);
//   auto vbar = round_pass(v);
//   auto vhat = vbar * s;
//   return vhat;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
  ...
  m.def("fixed_point_quantize_partial_nearest", &fixed_point_quantize_partial_nearest, "Fixed Point Number Nearest Neighbor Quantization (CPU)");
  m.def("fibonacci_quantize_nearest", &fibonacci_quantize_nearest, "Fixed Point Number Nearest Neighbor Quantization (CPU)");
  m.def("noAdjecentOnes_quantize_nearest", &noAdjecentOnes_quantize_nearest, "Fixed Point Number Nearest Neighbor Quantization (CPU)");
