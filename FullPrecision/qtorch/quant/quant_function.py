...
__all__ = [
    ...
    "fixed_point_quantize_partial",
    "fibonacci_quantize",
    "noAdjecentOnes_quantize",
    "LSQuantize",
    "block_quantize",
    "float_quantize",
    "quantizer",
]
# __all__ = ["fixed_point_quantize", "fibonacci_quantize", "block_quantize", "float_quantize", "quantizer"]
...
def fixed_point_quantize_partial(x, percentage, wl, fl, clamp=True, symmetric=False):
    """
    Quantize a single precision Floating Point into low-precision Fixed Point
    ...
    quant_module = get_module(x)
    out = quant_module.fixed_point_quantize_partial_nearest(
        x.contiguous(), wl, fl, clamp, symmetric, percentage
    )
    return out


def fibonacci_quantize(x, wl, fl, clamp=True, symmetric=False, rounding="nearest"):
    """
    Quantize a single precision Floating Point into low-precision Fixed Point
    ...
    assert isinstance(x, torch.Tensor)
    assert rounding in ["nearest"]
    assert_wl_fl(wl, fl)
    quant_module = get_module(x)
    if rounding == "nearest":
        out = quant_module.fibonacci_quantize_nearest(
            x.contiguous(), wl, fl, clamp, symmetric
        )
    return out


def noAdjecentOnes_quantize(x, wl, fl, clamp=True, symmetric=False, rounding="nearest"):
    """
    Quantize a single precision Floating Point into low-precision Fixed Point
    ...
    quant_module = get_module(x)
    if rounding == "nearest":
        out = quant_module.noAdjecentOnes_quantize_nearest(
            x.contiguous(), wl, fl, clamp, symmetric
        )
    return out

def LSQuantize(x, wl, fl, clamp=True, symmetric=False, rounding="nearest"):
    ...
    quant_module = get_module(x)
    if rounding == "nearest":
        out = quant_module.LS_quantize_nearest(
            x.contiguous(), wl, fl, clamp, symmetric
        )
    return out
