...
def get_model(config):
    ...
    model = GRU(config["enc_in"], config["enc_in"], config["hs"], config["hl"])
...
def run_validation(model, val_dataloader, device):
    ...
    plt.savefig("output_eval_GRU.png")
    plt.close()
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            if batch_idx % 100 == 0:
                ...
                plt.savefig("output_train_GRU.png")
                plt.close()
...
if __name__ == "__main__":
    ...
    config["model_folder"] = config["model_folder"] + "/GRU"
    config["experiment_name"] = (
        "/".join(config["experiment_name"].split("/", 1)) + "/GRU" + model_name
    )
    ...
    with open("loss_GRU.pkl", "wb") as f:
